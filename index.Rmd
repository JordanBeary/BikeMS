---
title: 'TUN Data Challenge 2018 : Bike MS'
...
* GitHub Webpage and final report : [JordanBeary](https://github.com/JordanBeary/BikeMS)
* Data cleansing and manipulation, data sets, and storing miscellaneous files : [konradmiz](https://github.com/konradmiz/ETM-527)

# Introduction

The National Multiple Sclerosis Society (NMSS), founded in 1946, is the 70th largest nonprofit in the United States^[Link to Forbes article [Forbes.com](https://www.forbes.com/companies/national-multiple-sclerosis-society/)], and is dedicated to fund research and perform outreach/public awareness and education regarding Multiple Sclerosis. Multiple Sclerosis (MS), is more frequent in women than men, and is typically diagnosed in patients between the ages of 20 and 50^[Read about Multiple Sclerosis at [NMSS.org](https://www.nationalmssociety.org/About-the-Society/MS-Prevalence)]. The causal mechanism of MS is unknown but hypothesized to be an interplay of an unknown environmental factor and genetic predisposition. Around 400,000 people in the United States have MS (with an additional 200 diagnosed weekly), and the frequency is about twice as high above the 37th parallel as below it. As such, Multiple Sclerosis represents a relevant, impactful disease with a need for research and education^[Quick MS statistics [Healthline](https://www.healthline.com/health/multiple-sclerosis/facts-statistics-infographic)].

The National Multiple Sclerosis Society raises funds primarily through individual donations and through sponsored charity bicycle events (Bike MS) which typically last one or two days^[Visit TUN Data Challenge homepage to look at business questions and competition info [TUN Data Challenge 2018](http://www.teradatauniversitynetwork.com/Community/Student-Competitions/2018/Data-Challenge/Business-Questions/)]. These events occur in the spring, summer, and fall, and about 70,000 participants take part in one of about 75 different events across the country. As the largest charity bike fundraiser in the United States, the events collectively raise about `$`68 million. Teams are the largest contributor to the donation pot, representing about 87% of the money raised. However, as charities representing many distinct pathologies now rely on individual donors and bike rides, the market share for Bike MS has fallen yearly since 2012 and is reflected in declining revenue and numbers of participants. Retention of past participants is strong, but fewer and fewer new participants are joining. In 2017, the NMSS suffered a loss of $8 million^[Link to Forbes article [Forbes.com](https://www.forbes.com/companies/national-multiple-sclerosis-society/)]. To maintain a fundraising advantage and remain competitive in fundraising, the NMSS has partnered with Teradata University Network to provide their data and relevant business questions to be addressed.


Multiple Sclerosis defined by the National Multiple Sclerosis Society^[Read about Multiple Sclerosis at [NMSS.org](https://www.nationalmssociety.org/About-the-Society/MS-Prevalence)]:
...
```{r fortune, eval=FALSE, include=FALSE}
participants_joined <- readRDS("C:\\Users\\J-Blazer\\Documents\\ETM 527\\Bike MS\\Fortune500 Matching.rds")

f500 <- read_csv("C:\\Users\\J-Blazer\\Documents\\ETM 527\\Bike MS\\Fortune 1000 Donations.csv")

f500$Company <- toupper(f500$Company)

f500$Company <- gsub("CORPORATION", "", f500$Company)
f500$Company <- gsub("COMPANY", "", f500$Company)
f500$Company <- gsub("INCORPORATED", "", f500$Company)
f500$Company <- gsub("THE", "", f500$Company)
f500$Company <- gsub(" CORP", "", f500$Company)
f500$Company <- gsub(" INC", "", f500$Company)
f500$Company <- gsub(" CO", "", f500$Company)

f500$Company <- trimws(f500$Company, which = "both")

participants_joined$fortune500 <- NA

fortune_max <- vector(mode = "numeric", length = nrow(f500))


for (i in 1:nrow(participants_joined)) {
  for (k in 1:nrow(f500)) {
    fortune_max[] <- 0
    if (RecordLinkage::jarowinkler(participants_joined$`Participant Employer`[i],  f500$Company[k]) > 0.9) {
      fortune_max[k] <- RecordLinkage::jarowinkler(participants_joined$`Participant Employer`[i],  f500$Company[k])
      
      participants_joined$fortune500[i] <- f500$Company[which(fortune_max == max(fortune_max))]
    }
  }
...
check_match <- participants_joined %>%
  select(`Participant Employer`, fortune500) %>%
  filter(!is.na(fortune500))

f500_part <- participants_joined %>%
  filter(!is.na(fortune500))

not_f500 <- dplyr::setdiff(participants_joined, f500_part)


ggplot(f500_part, aes(`Total of All Confirmed Gifts($)`+ 1)) +
  geom_density(color = "blue", size = 0.8, alpha = 0.5) + 
  scale_y_log10() + 
  geom_density(data = not_f500, aes(`Total of All Confirmed Gifts($)`+ 1), color = "red", size = 0.8, alpha = 0.5)
```
...
```{r facet.participants, fig.width = 15, fig.height = 6, fig.fullwidth = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
ggplot(state.occ) +
...
```{r gify, message=FALSE, warning=FALSE}
# growth of MS 
...
```{r events, message=FALSE, warning=FALSE, include=FALSE}
BikeEvents <- read_csv("C:\\Users\\J-Blazer\\OneDrive\\ETM 527 DataSets\\Events\\2013-2017 Bike Events.csv")
...
BikeEvents$`Event Dat` <- mdy_hm(BikeEvents$`Event Date`)
BikeEvents$`Event Date` <- as.Date(BikeEvents$`Event Dat`)
BikeEvents$Month <- format(BikeEvents$`Event Date`, "%B")
BikeEvents$Month <- as.factor(BikeEvents$Month)


bestbudsHP <- BikeEvents %>%
  select(`Fiscal Year`, `Security Category Name`,`Event Date`, City, State, `Total of All Confirmed Gifts($)`, 
         `Active Registrations`, Month) %>%
  group_by(`Fiscal Year`, `Month`, State) %>%
  filter(Month == "June")

HPradius <- c("MA", "RI", "DE", "MD", "NY", "PA", "CT")

bestbudsHP.ts <- bestbudsHP %>%
  select(everything()) %>%
  group_by(`State`, `Event Date`) %>%
  filter(State %in% HPradius) %>%
...
[MS Gender Gap]: MSgenderGap.jpg
[Participation Interaction between Competing Events]: participantsfromtopmarkets.png
[Fundraising Interaction between Competing Events]: fundsraisedbytopmarkets.png
[Competing Events]: competingevents.png
[Common Denominator]: commondenominator.png
[Participation of Top National Teams]: nationalteams2.png
[Number of Teams per National Team]: nationalteams3.png
[Top Donating Occupations]: topOccupationsdonation.png
[Involvement of Top National Teams]: nationalteams1.png
[Prevalence Giffy]: MS.gif
[Digital Marketing]: digitalmarketing.png
[Fortune 1000]: fortune1000.png
[Fortune 500]: fortune500.png